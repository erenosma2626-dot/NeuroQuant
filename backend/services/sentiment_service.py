...
def get_global_macro_news(max_results: int = 10) -> Dict[str, Any]:
    """
    ...
    """
    cache_key = "news_sentiment_global_macro_v3"
    cached = cache.get(cache_key)
    if cached:
        return cached
    ...
    riskiest = next((n for n in news_items if n["score"] <= -0.4), None)
    catalyst = next((n for n in news_items if n["score"] >= 0.4), None)
